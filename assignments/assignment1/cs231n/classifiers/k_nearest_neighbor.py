...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i, j] = np.sqrt(np.sum(np.power(d, 2.0)))
        return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            d = self.X_train - X[i, :]
            dists[i, :] = np.linalg.norm(d, axis=1)
        return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train))
        # Using that
        # sum((x-y)^2) = -2*sum(xy) + sum(x^2) + sum(y^2)
        dists = -2.0 * (X @ self.X_train.T) +\
            np.sum(np.power(self.X_train, 2.0),
                   axis=1).reshape(1, num_train) +\
            np.sum(np.power(X, 2.0), axis=1).reshape(num_test, 1)
        dists = np.sqrt(dists)
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            y_pred[i] = cls[np.argmax(counts)]
            # (first occurance of max count corresponds to the smaller class)
